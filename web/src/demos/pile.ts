...
/** Particles added by the initial drop and by each +100. */
...
const DROP_HEIGHT = 6;
const START_X = (-(SIDE - 1) * SPACING) / 2;
/**
 ...
 */
const DAMPING = 0.1;
/** The centre of the ground, which every particle is drawn to. */
const FUNNEL = { x: 0, y: 0, z: 0 };
const FUNNEL_STRENGTH = 1;
...
export class PileDemo implements Demo {
  ...
  readonly actions: DemoAction[] = [{ label: '+100', key: 'x', run: () => this.drop() }];
  ...
  get hint(): string {
    return `${this.engine.particleCount()} / ${this.engine.capacity()} particles; +100 drops another batch`;
  }
  ...
  private drop(): void {
    for (let i = 0; i < BATCH; i++) {
      ...
      const col = i % SIDE;
      const position = {
        x: START_X + col * SPACING + jitter(),
        y: DROP_HEIGHT + layer * SPACING,
        z: START_X + row * SPACING + jitter(),
      };
      const p = this.engine.addParticle(position, 1, DAMPING, RADIUS);
      if (p < 0) return;
      this.engine.addAnchoredSpring(p, FUNNEL, FUNNEL_STRENGTH, 0);
    }
  }
}
